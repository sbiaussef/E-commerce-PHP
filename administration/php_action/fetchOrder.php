...
if($result->num_rows > 0) { 
 ...
 while($row = $result->fetch_array()) {
 	...
 	// active 
 	if($row[4] == 1) { 		
 		$paymentStatus = "<label class='label label-success'> الدفع كامل</label>";
 	} else if($row[4] == 2) { 		
 		$paymentStatus = "<label class='label label-info'>الدفع مقدماً</label>";
 	} else { 		
	...
	<div class="btn-group">
	  <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
	    تحديد <span class="caret"></span>
	  </button>
	  <ul class="dropdown-menu">
	    <li><a href="orders.php?o=editOrd&i='.$orderId.'" id="editOrderModalBtn"> <i class="glyphicon glyphicon-edit"></i> تعديل</a></li>
	    
	    <li><a type="button" data-toggle="modal" id="paymentOrderModalBtn" data-target="#paymentOrderModal" onclick="paymentOrder('.$orderId.')"> <i class="glyphicon glyphicon-save"></i> دفع</a></li>
