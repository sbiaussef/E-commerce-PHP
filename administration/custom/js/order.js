...
$(document).ready(function() {
	...
	if(divRequest == 'add')  {
		...
		$("#createOrderForm").unbind('submit').bind('submit', function() {
			...
			// form validation 
			if(orderDate == "") {
				$("#orderDate").after('<p class="text-danger"> يجب أختيار التاريخ </p>');
			...
			if(clientName == "") {
				$("#clientName").after('<p class="text-danger">يجب إدخال أسم العميل </p>');
			...
			if(clientContact == "") {
				$("#clientContact").after('<p class="text-danger"> يجب إدخال وسيلة التواصل</p>');
			...
			if(paid == "") {
				$("#paid").after('<p class="text-danger"> يجب إدخال قيمة المبلغ المدفوع </p>');
			...
			if(discount == "") {
				$("#discount").after('<p class="text-danger"> يجب إدخال قيمة الخصم </p>');
			...
			if(paymentType == "") {
				$("#paymentType").after('<p class="text-danger"> يجب تحديد وسيلة الدفع </p>');
			...
			if(paymentStatus == "") {
				$("#paymentStatus").after('<p class="text-danger">يجب تحديد حالة الدفع </p>');
		    ...
		    if(productName[x].value == ''){	    		    	
		    	$("#"+productNameId+"").after('<p class="text-danger">يجب إدخال أسم المنتج </p>');
	   	...
	   	for (var x = 0; x < quantity.length; x++) {       
		    ...
		    if(quantity[x].value == ''){	    	
		    	$("#"+quantityId+"").after('<p class="text-danger"> يجب تحديد الكمية  </p>');
	   	...
	   	for (var x = 0; x < quantity.length; x++) {       						
			...
			if(orderDate && clientName && clientContact && paid && discount && paymentType && paymentStatus) {
				if(validateProduct == true && validateQuantity == true) {
	            	...
	            	' <br /> <br /> <a type="button" onclick="printOrder('+response.order_id+')" class="btn btn-primary"> <i class="glyphicon glyphicon-print"></i> طباعة </a>'+
	            	'<a href="orders.php?o=add" class="btn btn-default" style="margin-left:10px;"> <i class="glyphicon glyphicon-plus-sign"></i> إضافة طلب جديد </a>'+
...
function paymentOrder(orderId = null) {
	if(orderId) {
		...
		$.ajax({
			...
			success:function(response) {				
				...
				$("#updatePaymentOrderBtn").unbind('click').bind('click', function() {
					...
					if(payAmount == "") {
						$("#payAmount").after('<p class="text-danger">يجب إدخال قيمة المبلغ المدفوع</p>');
		...
		alert('خطأ !! يجب عمل تحديث للصفحة');
	}
}
